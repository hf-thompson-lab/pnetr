...
SensitivityAnalysis <- function(clim_dt, 
    ...
    if (!is.null(cl)) {
        parallel::clusterExport(cl, c(
                "vegpar_new", "clim_dt", "vegpar_templ", "vegpar_mat", 
                "sitepar_templ", "sitepar_mat", "model", "target_pars",
                "annual"
            ), envir = environment()
        )
        veg_res_dt <- parallel::clusterApplyLB(cl, 
            x = 2:ncol(vegpar_mat), 
            fun = function(pars_ind) {
                pars <- vegpar_mat[, pars_ind]
                for (i in 1:length(vegpar_mat[, 1])) {
                    vegpar_new[[vegpar_mat[i]]] <- as.numeric(pars[i])
                }

                out <- switch(tolower(model),
                    "pnet-day" = PnET_Day(clim_dt, sitepar_templ, vegpar_new),
                    "pnet-ii" = PnET_II(clim_dt, sitepar_templ, vegpar_new),
                    "pnet-cn" = PnET_CN(clim_dt, sitepar_templ, vegpar_new)
                )
    ...
    if (!is.null(cl)) {
        ...
        site_res_dt <- parallel::clusterApplyLB(cl, 
            x = 2:ncol(sitepar_mat), 
            fun = function(pars_ind) { 
                pars <- sitepar_mat[, pars_ind]
                ...
                out <- switch(tolower(model),
                    "pnet-day" = PnET_Day(clim_dt, sitepar_new, vegpar_templ),
                    "pnet-ii" = PnET_II(clim_dt, sitepar_new, vegpar_templ),
                    "pnet-cn" = PnET_CN(clim_dt, sitepar_new, vegpar_templ)
                )
        ...
        site_res_dt <- apply(sitepar_mat[, -1], 2, function(pars) {
            ...
            out <- switch(tolower(model),
                "pnet-day" = PnET_Day(clim_dt, sitepar, vegpar_templ),
                "pnet-ii" = PnET_II(clim_dt, sitepar, vegpar_templ),
                "pnet-cn" = PnET_CN(clim_dt, sitepar, vegpar_templ)
            )
